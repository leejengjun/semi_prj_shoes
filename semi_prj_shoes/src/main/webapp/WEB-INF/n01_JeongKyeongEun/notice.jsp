...
<script type="text/javascript">
   ...
   $(document).ready(function(){
      ...
      $("select#sizePerPage").bind("change", function() {
			const frm = document.noticeSearchFrm
			frm.action = "notice.shoes";
			frm.method = "get";
			frm.submit();
		});
		...
		// 검색하기
		$("form[name='noticeSearchFrm']").submit(function(){  // 이 폼이 submit 될때 검사하겠다.   검색
...
// Function Declaration 
	function goSearch() {
		...
		if( $("select#searchType").val() == "" ) {
	 	...
	 	const frm = document.noticeSearchFrm;
	 	frm.action = "notice.shoes";  // 자기가 자기한테 가기
	 	...
	 	frm.submit();
			...
			<!-- --------------------------- main --------------------------------------- -->
		<main class="main">
			<h2 class="main title" style="margin-top: 40px; padding-left: 40px; font-weight: bold; ">공지사항</h2>
			
			<div class="search-form first align-right">
				
				<form name="noticeSearchFrm" action="notice.shoes" method="get">
				...
				<table class="table">
					...
					<tbody> <!-- 여기부터 글 목록 -->
					 	<c:if test="${not empty requestScope.noticeList}">
							<c:forEach var="bvo" items="${requestScope.noticeList}">	 
								<tr class="noticeList">
									<td class="notice_no">${bvo.notice_no}</td>
									<td class="title indent text-align-left"><a style="color: #212529;" href="<%=ctxPath%>/notice_contents.shoes?notice_no=${bvo.notice_no}">${bvo.n_title}</a></td>
									<td>${bvo.n_userid}</td>
									<td>${bvo.n_date}</td>
								</tr>
							</c:forEach>
			...
			<c:if test="${not empty sessionScope.loginuser and sessionScope.loginuser.userid eq 'admin'}">   
				<div style="border: solid 0px red; width: 100%;" align="right">
					<button type="button" class="btn btn-secondary btn-sm my-3" onclick="javascript:location.href='<%= ctxPath%>/notice_write2.shoes'">글쓰기</button>
				</div>
			</c:if>
