...
<script type="text/javascript">
  ...
  $(document).ready(function(){
	  ...
	  $("button#btnEdit").click(function(){
		...
		if( $("textarea[name=n_contents]").val().trim() == "" ) {
			alert("글 내용을 입력하세요.");
			return;
		}
		
		if( parseInt($("input[name=n_title]").val().length) > 100 ) {
			alert("글 제목은 100자까지 가능합니다.");
			return;
		}
		
		if( parseInt($("textarea[name=n_contents]").val().length) > 4000 ) {
			alert("글 내용은 4000자까지 가능합니다.");
			return;
		}
			
		const frm = document.noticeEditFrm;
		frm.action = "<%= ctxPath%>/noticeEditEnd.shoes";
		frm.method = "post";
		frm.submit();
...
<body>
	...
	<div class="container" style="margin-bottom: 50px;">	
		...
		<!------- 글쓰기 폼태그 위에 공지사항 큰 글씨 시작 ------->
		<div class="col-md-12 text-left">
		    <br>
		  	<h2 style="color: black; font-weight: bolder;">&nbsp;&nbsp;공지사항</h2>
			<br>
		</div>
	...
	<!-- 글쓰기 폼태그 수정 시작 -->
		<div class="container" role="main">
			<form name="noticeEditFrm" enctype="multipart/form-data">
				...
				<input type="hidden" style="width: 100%" name="n_userid" value="admin" />  <%-- ${requestScope.bvo.n_userid} 로 해도 안 됨--%>
					...
					<tr>
				        <td bgcolor=white>
					        <table class = "table2">
								...
								<tr>
					                <td>제목</td>
					                <td><input name="n_title" type="text" size="101" maxlength="100" required autofocus value="${requestScope.bvo.n_title}" /></td>
						        </tr>
								<!-- 글내용 쓰기!!!  -->
								<tr>
					                <td>글내용</td>
					                <c:if test="${not empty requestScope.bvo.n_file}">
					                	<td><textarea name="n_contents" maxlength="4000" cols="103" rows="17"><img src="/semi_prj_shoes/images/JeongKyeongEun/${requestScope.bvo.n_file}" class="img-fluid" style="width: 100%;" />${requestScope.bvo.n_contents}</textarea></td>    
					                </c:if>  
					                <c:if test="${empty requestScope.bvo.n_file}">
										<td><textarea name="n_contents" maxlength="4000" cols="103" rows="17">${requestScope.bvo.n_contents}</textarea></td>
									</c:if> 
								</tr>
								<!-- <textarea rows="10" cols="50" name="n_contents"></textarea>  강사님이 테스트 해주신 것 -->
								<!-- 파일 첨부하기!!!  -->
								<tr>
					                <td>첨부파일</td>
					                <td><input type="file" name="n_file" accept=".jpg, .png"/></td>
					           	</tr>
				...
				<p class="text-center" style="margin-top: 30px; width: 100%;">
					...
					<button type="button" class="btn btn-secondary btn-md" onClick="location.href='<%=ctxPath%>/notice.shoes'">목록</button>
				</p>
				<!-- 수정 취소 목록 버튼 끝 -->
