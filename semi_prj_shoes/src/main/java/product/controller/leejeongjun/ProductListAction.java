...
public class ProductListAction extends AbstractController {
	...
	//		System.out.println("확인용 searchWord " +searchWord);
			...
			// 메뉴에서 상품관리 만을 클릭했을 경우에는 currentShowPageNo 은 null 이 된다.
			// currentShowPageNo 이 null 이라면 currentShowPageNo 을 1 페이지로 바꾸어야 한다.
			
			String sizePerPage = request.getParameter("sizePerPage");
			// 한 페이지당 화면상에 보여줄 상품의 개수
			// 메뉴에서 삼품관리 만을 클릭했을 경우에는 sizePerPage 은 null 이 된다.
			...
			// 검색이 있을 경우 시작 //
			if(searchType != null && !"".equals(searchType) && !"pname".equals(searchType) && !"pnum".equals(searchType)) {
				// 사용자가 웹브라우저 주소입력란에서 searchType 란에 장난친 경우 
				String message = "부적절한 검색 입니다. 장난치지 마세요.";
				String loc = request.getContextPath()+"/admin/productList.shoes";
		...
		//	System.out.println("~~~ 확인용 productList.size() " + productList.size());
		    ...
		    */
			...
			// **** [맨처음][이전] 만들기 **** //
			if( pageNo != 1 ) {
				pageBar += "<li class='page-item'><a class='page-link' href='productList.shoes?currentShowPageNo=1&sizePerPage="+sizePerPage+"&searchType="+searchType+"&searchWord="+searchWord+"'>[맨처음]</a></li>";		
				pageBar += "<li class='page-item'><a class='page-link' href='productList.shoes?currentShowPageNo="+(pageNo-1)+"&sizePerPage="+sizePerPage+"&searchType="+searchType+"&searchWord="+searchWord+"'>[이전]</a></li>";
			}
			...
			while( !(loop > blockSize || pageNo > totalPage ) ) {
				...
				if( pageNo == Integer.parseInt(currentShowPageNo) ) {
					...
					pageBar += "<li class='page-item'><a class='page-link' href='productList.shoes?currentShowPageNo="+pageNo+"&sizePerPage="+sizePerPage+"&searchType="+searchType+"&searchWord="+searchWord+"'>"+pageNo+"</a></li>";
				}
			...
			// pageNo ==> 11
			if(pageNo <= totalPage) {
				pageBar += "<li class='page-item'><a class='page-link' href='productList.shoes?currentShowPageNo="+pageNo+"&sizePerPage="+sizePerPage+"&searchType="+searchType+"&searchWord="+searchWord+"'>[다음]</a></li>";
				pageBar += "<li class='page-item'><a class='page-link' href='productList.shoes?currentShowPageNo="+totalPage+"&sizePerPage="+sizePerPage+"&searchType="+searchType+"&searchWord="+searchWord+"'>[마지막]</a></li>";
			}
			...
			// 회원조회를 했을 시 현재 그 페이지 그대로 되돌아가기 위한 용도로 쓰임.
		...
		//	super.setRedirect(false);
			super.setViewPage("/WEB-INF/n01_leejeongjun/adminPage/admin_productPage.jsp");
		}
