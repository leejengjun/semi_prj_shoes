...
@WebServlet(
		description = "사용자가 웹에서 *.shoes 을 했을경우 이 서블릿이 응답을 해준다.", 
		urlPatterns = { "*.shoes" }, 
		initParams = { 
				@WebInitParam(name = "propertyConfig", value = "C:/Users/sist/git/semi_prj_shoes/semi_prj_shoes/src/main/webapp/WEB-INF/Command.properties", description = "*.shoes 에 대한 클래스의 매핑파일")
		})
public class FrontController extends HttpServlet {
	
	private static final long serialVersionUID = 1L;

	Map<String, Object> cmdMap = new HashMap<>();	//필드 생성, 저장소
	
	
	public void init(ServletConfig config) throws ServletException {
	/*
		웹브라우저 주소창에서  *.shoes 을 하면 FrontController 서블릿이 응대를 해오는데 
	...
	*/
	...
	//	System.out.println("~~~ 확인용 props => "+props);
		// ~~~ 확인용 props => C:\Users\sist\git\semi_prj_shoes\semi_prj_shoes\src\main\webapp\WEB-INF\Command.properties

		try {
			fis = new FileInputStream(props);
			// fis 는 C:\Users\sist\git\semi_prj_shoes\semi_prj_shoes\src\main\webapp\WEB-INF\Command.properties 파일의 내용을 읽어오기 위한 용도로 쓰이는 객체이다.
			...
			pr.load(fis);
			/*
				pr.load(fis); 은  fis 객체를 사용하여 C:\Users\sist\git\semi_prj_shoes\semi_prj_shoes\src\main\webapp\WEB-INF\Command.properties 파일의 내용을 읽어다가 
			...
			*/
			...
			/*
				pr.keys(); 은
				C:\Users\sist\git\semi_prj_shoes\semi_prj_shoes\src\main\webapp\WEB-INF\Command.properties 파일의 내용물에서 
			...
			*/
			...
			System.out.println(">>> C:/Users/sist/git/semi_prj_shoes/semi_prj_shoes/src/main/webapp/WEB-INF/Command.properties 이 없습니다! <<<");
			e.printStackTrace();
		} catch (IOException e) {
			e.printStackTrace();
	...
	//	System.out.println("~~~ 확인용 key =>"+key);
		...
		AbstractController action = (AbstractController) cmdMap.get(key);	// 맵에 저장된 객체를 불러옴.
		...
		if(action == null) {
			...
			try {
				...
				String viewPage = action.getViewPage();	// 뭐뭐.jsp	또는 뭐뭐.shoes
