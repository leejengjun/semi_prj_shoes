...
public class NoticeEditAction extends AbstractController {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// == 관리자(admin)로 로그인 했을 때만 글수정이 가능하도록 해야 한다. == //
...
//		    System.out.println("=== 첨부되어지는 이미지 파일이 올라가는 절대경로 uploadFileDir ==> " + uploadFileDir);
		...
		//	String n_file = mtrequest.getFilesystemName("n_file");  // 실제 파일서버에 올라간 파일명!!!!!!
			...
			n_contents = n_contents.replaceAll("&lt;br&gt;", "\r\n"); // !!!!!!! 
			// 이렇게 하지 않으면 글 수정시 나오는 글내용 중 엔터가 <br>로 입력되어져 나온다.!!! 주의 !!!
			...
			super.setViewPage("/WEB-INF/n01_JeongKyeongEun/notice_edit.jsp");
		}
