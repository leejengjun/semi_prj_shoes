...
public class EventAction extends AbstractController {
	...
	InterBoardEventDAO bdao = new BoardEventDAO();		// DAO 에서 가져온다.
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// 메뉴에서 이벤트게시판 만을 클릭했을 경우에는 currentShowPageNo 은 null 이 된다.
		// currentShowPageNo 이 null 이라면 currentShowPageNo 을 1 페이지로 바꾸어야 한다.
		
		String sizePerPage = request.getParameter("sizePerPage"); 
		// 한 페이지당 화면상에 보여줄 글의 개수
		// 메뉴에서 이벤트게시판만을 클릭했을 경우에는 sizePerPage 는 null 이 된다.
		...
		// 검색이 있을 경우 시작 //
		if(searchType != null && !"".equals(searchType) && !"e_title".equals(searchType) && !"e_userid".equals(searchType)) {
			// 사용자가 웹브라우저 주소입력란에서 searchType 란에 장난친 경우
			// 유저가 장난쳐 온 경우이기 때문에 이 밑으로 내려가면 안 된다. 여기서 끝냄
			String message = "부적절한 검색입니다.";
			String loc = request.getContextPath()+"/event.shoes";  // 이벤트게시판 목록페이지로 이동
	...
	//	System.out.println("확인용 totalPage : " + totalPage);
		// 확인용 totalPage : 21  검색 안 할때 총 페이지 수
		// 확인용 totalPage : 11  '유' 검색했을때 총 페이지 수
		...
		// 이벤트게시판 목록 조회 & 페이징 처리가 된 이벤트게시판 글 또는 검색한 글 목록
		List<BoardEventVO> eventList = bdao.selectPagingEvent(paraMap); // 테이블에 기록된 모든 글들을 조회해온다.		 	
				
		request.setAttribute("eventList",  eventList);		// view 단으로 넘겨서 출력한다.
        ...
        1개 페이지당 3개행 또는 5개행 또는  10개행을 보여주는데
            ...
            총 몇개 블럭이 나와야 할까? 
            총 게시글이 207명 이고, 1개 페이지당 보여줄 게시글이 5 이라면
	...
	// ==== !!! pageNo 구하는 공식 !!! 테이블 행의 개수가 아니라 밑에 페이지바에 표시되는 숫자의 개수이다. ==== // 
       ...
       */
		...
		// **** ============ [맨처음] [이전] 만들기 =========== **** // 
		if(pageNo != 1) {
		pageBar += "<li class='page-item'><a class='page-link' href='event.shoes?currentShowPageNo=1&sizePerPage="+sizePerPage+"&searchType="+searchType+"&searchWord="+searchWord+"'>[맨처음]</a></li>";
		pageBar += "<li class='page-item'><a class='page-link' href='event.shoes?currentShowPageNo="+(pageNo-1)+"&sizePerPage="+sizePerPage+"&searchType="+searchType+"&searchWord="+searchWord+"'>[이전]</a></li>";
		}
		...
		while( !(loop > blockSize || pageNo > totalPage) ) { // 10번 반복
			...
			if(pageNo == Integer.parseInt(currentShowPageNo)) { // 내가 선택한 번호 가 현재 보여주는 페이지 번호와 같다면
				...
				pageBar += "<li class='page-item'><a class='page-link' href='event.shoes?currentShowPageNo="+pageNo+"&sizePerPage="+sizePerPage+"&searchType="+searchType+"&searchWord="+searchWord+"'>"+pageNo+"</a></li>";				
			}
			loop++;
			pageNo++;
		}// end of while----------------------------------------------
		...
		if(pageNo <= totalPage ) { // 마지막 페이지가 아닐 때만 보여준다.
			pageBar += "<li class='page-item'><a class='page-link' href='event.shoes?currentShowPageNo="+pageNo+"&sizePerPage="+sizePerPage+"&searchType="+searchType+"&searchWord="+searchWord+"'>[다음]</a></li>";			
			pageBar += "<li class='page-item'><a class='page-link' href='event.shoes?currentShowPageNo="+totalPage+"&sizePerPage="+sizePerPage+"&searchType="+searchType+"&searchWord="+searchWord+"'>[마지막]</a></li>";
		}
		...
		// 글을 조회 했을시 현재 그 페이지로 그대로 되돌아가기 위한 용도로 쓰임.
		
	//	System.out.println("확인용 currentURL => " +currentURL);
		/*
		 확인용 currentURL => /event.shoes
		 확인용 currentURL => /event.shoes?sizePerPage=3&searchWord=&searchType=
	...
	//	super.setRedirect(false);
		super.setViewPage("/WEB-INF/n01_JeongKyeongEun/event.jsp");
